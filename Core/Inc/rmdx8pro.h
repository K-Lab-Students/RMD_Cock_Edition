...
#define GYEMS_SPEED_PER_NUMBER      1 //dps/LSB
...
#define MOTOR_REDUCTION_RATIO 9
...
typedef enum {
	...
	READ_ENCODER_POS = 0x60,
	READ_ENCODER_POS_ORIG = 0x61,
	READ_ENCODER_POS_ZERO = 0x62,
...
void GYEMSDevicePositionControlCmd4(GYEMSDevice_t* device_ptr, float unsignedPosDegree,
        float unsignedMaxSpeed, int spinDirection);

void GYEMSFrameParser(CanFrame_t* canFrame);
void motorInit(MotorValues_t* motor, uint16_t idEntered);
void motor_receive_pos(MotorValues_t* motor, uint8_t dataReceived[8]);
